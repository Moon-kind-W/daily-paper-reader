...
tags: ["query:agmem"]
score: 10.0
evidence: 用于扩展长程智能体的索引经验记忆
tldr: 针对大语言模型在长周期任务中受限于上下文窗口的问题，本文提出 Memex 机制。该机制通过索引化经验记忆，将简洁摘要保留在工作上下文中，而将完整交互存储于外部数据库。配合 MemexRL 强化学习框架，模型学会自主管理记忆的存取。实验证明，该方法在显著减少上下文占用的同时，提升了长周期任务的成功率，为长程智能体提供了更高效的记忆管理方案。
source: arxiv
selection_source: fresh_fetch
motivation: 现有的上下文压缩方法（如截断或摘要）会导致原始证据丢失，限制了智能体在长周期任务中利用远端信息的能力。
method: 提出 Memex 索引记忆机制，结合外部数据库存储完整细节，并利用 MemexRL 强化学习框架优化智能体的总结、归档与检索决策。
result: 在挑战性的长周期任务中，Memex 智能体在维持较小工作上下文的同时，显著提高了任务成功率并降低了信息损失。
conclusion: 通过索引化经验记忆与强化学习相结合，可以有效解决 LLM 智能体的长程记忆瓶颈，实现高效且无损的信息检索与决策。
---

## 摘要
大语言模型 (LLM) 智能体在处理长程任务时，从根本上受到有限上下文窗口的限制。随着轨迹的增长，在上下文中保留工具输出和中间推理很快变得不可行：工作上下文变得过长，最终超出上下文预算，并使得即使仍然存在的远程证据也难以被利用。现有的解决方案通常通过截断或滚动摘要来缩短上下文，但这些方法从根本上是有损的，因为它们压缩或丢弃了过去的证据本身。我们引入了 Memex，这是一种索引经验记忆机制，它在不丢弃证据的情况下压缩上下文。Memex 维护一个由简洁结构化摘要和稳定索引组成的紧凑工作上下文，同时将高保真的底层交互存储在这些索引下的外部经验数据库中。智能体随后可以决定何时解引用索引，并恢复当前子目标所需的精确历史证据。我们通过强化学习框架 MemexRL 优化读写行为，使用针对上下文预算下索引记忆使用情况定制的奖励塑造，使智能体学会总结什么、归档什么、如何索引以及何时检索。与仅使用摘要的方法相比，这产生了一种损耗显著更低的长程记忆形式。我们进一步提供了理论分析，展示了 Memex 循环在保持有效上下文计算量随历史增长而有界的同时，通过有界的解引用来保持决策质量的潜力。实验表明，在具有挑战性的长程任务中，使用 MemexRL 训练的 Memex 智能体在显著减小工作上下文的同时，提高了任务成功率。
